...
-- Essais d'entraînement (repasser un examen déjà terminé) : contrairement à
-- mock_exam_results, on ne garde QUE le score, jamais les réponses données
-- (gain de place — voir migration_mock_exam_retakes.sql). mode='full' pour
-- repasser toutes les questions, 'wrong_only' pour ne repasser que celles
-- ratées (ou non répondues) au tout premier essai officiel.
...
-- Pas de policy INSERT/UPDATE/DELETE sur mock_exam_attempts : les écritures
-- ne passent que par submit_mock_exam_attempt (SECURITY DEFINER, bypass RLS
-- en interne), donc personne ne peut insérer un score bidon directement.
...
-- Répartition par thème de TOUS les participants ayant déjà soumis (pas
-- juste l'appelant), pour comparer les points faibles/forts par catégorie
-- entre inscrits. N'expose que des agrégats (correct/total/pct) par thème,
-- jamais le détail question par question des autres.
...
CREATE INDEX IF NOT EXISTS mock_exam_attempts_exam_user ON mock_exam_attempts(exam_id, user_id, completed_at DESC);

-- ----------------------------------------------------------------
-- 7bis. MINI-SESSIONS D'ENTRAÎNEMENT (topics choisis, poids officiels)
-- ----------------------------------------------------------------
-- Voir migration_practice_sessions.sql pour le détail. Comme pour les
-- essais d'entraînement, seul le score est conservé en historique, jamais
-- les réponses.
...
CREATE TABLE IF NOT EXISTS practice_session_results (
  ...
  total            int         NOT NULL,
  duration_seconds int,
  -- Réponses données ([{"question_id","selected_index"}, ...]) pour
  -- reconstruire la correction à la demande bien après coup — voir
  -- get_practice_session_review / migration_practice_session_persist_and_xp.sql.
  answers          jsonb,
  completed_at     timestamptz DEFAULT now()
);
...
BEGIN
  ...
  -- Anti-farming partagé avec QCM/exercices/etc. : seules les questions
  -- réussies et jamais créditées avant (tous modes confondus) rapportent
  -- du XP.
  credited AS (
    INSERT INTO quiz_question_progress (user_id, question_id)
...
-- profil public, dans le même esprit que les trophées et l'XP. N'expose
-- que ce qu'il faut pour tracer la courbe (topics/format/score/total/date)
-- — jamais answers/review (get_practice_session_review continue de
-- vérifier user_id = auth.uid(), le contenu reste privé).
